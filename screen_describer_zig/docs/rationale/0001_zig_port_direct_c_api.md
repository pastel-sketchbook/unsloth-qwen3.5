...
- Streaming raw tokens to stdout during generation produces unreadable markdown syntax (visible `**`, `#`, `` ` `` markers). The fix is to buffer all tokens silently, then render the final text through the ANSI markdown renderer. Progress feedback during generation is sent to stderr as dot indicators (one dot per 32 tokens), keeping stdout reserved for the styled final output. This change was applied to both the Zig and Rust implementations.
